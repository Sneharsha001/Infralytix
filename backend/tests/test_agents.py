...
@pytest.fixture
def user_a():
    return User(id=uuid.uuid4(), name="Alice", email="alice@test.io", hashed_password="x",
                role=UserRole.USER, is_active=True, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))

@pytest.fixture
def user_b():
    return User(id=uuid.uuid4(), name="Bob", email="bob@test.io", hashed_password="x",
                role=UserRole.USER, is_active=True, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
...
def sample_project(user_a):
    return Project(id=uuid.uuid4(), user_id=user_a.id, name="AI Gateway", description="ML API",
                   repo_name="ai-gw", archive_filename="repo.zip",
                   created_at=datetime.now(UTC), updated_at=datetime.now(UTC))


SAMPLE_REPO_ANALYSIS = {
    "total_files": 42, "total_loc": 8500, "primary_language": "Python",
    "languages": [{"language": "Python", "extension": ".py", "file_count": 38, "loc": 8000, "percentage": 94.1}],
    "dependency_files": [{"filename": "requirements.txt", "package_manager": "pip", "dependencies": ["fastapi", "pytest"]}],
    "detected_frameworks": ["FastAPI", "pytest"],
}
GEMINI_OUTPUT = {
    "summary": "Mock AI insight.",
    "health_score": {"overall": 72, "maintainability": 75, "complexity": 70, "test_coverage_estimate": 65, "documentation": 55},
    "insights": [], "tech_debt_indicators": [], "recommended_next_steps": [],
}


def _repo_run(proj):
    r = AgentRun(id=uuid.uuid4(), project_id=proj.id, agent_type="repository",
                 status=AgentRunStatus.COMPLETED, output_data=SAMPLE_REPO_ANALYSIS,
                 error_message=None, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
    r.project = proj
    return r


def _ai_run(proj):
    r = AgentRun(id=uuid.uuid4(), project_id=proj.id, agent_type="gemini",
                 status=AgentRunStatus.COMPLETED, output_data=GEMINI_OUTPUT,
                 error_message=None, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
    r.project = proj
    return r
...
class TestOwnership:
    def _fp(self, user_b):
        return Project(id=uuid.uuid4(), user_id=user_b.id, name="B", description=None,
                       repo_name=None, archive_filename=None,
                       created_at=datetime.now(UTC), updated_at=datetime.now(UTC))

    def test_analyze_wrong_owner_404(self, client, user_a, user_b, auth_header):
        fp = self._fp(user_b)
        with (patch("app.middlewares.auth_middleware.UserRepository") as ur,
            ...
            assert client.post(f"/api/v1/projects/{fp.id}/analyze", headers=auth_header).status_code == 404
    ...
    def test_analysis_wrong_owner_404(self, client, user_a, user_b, auth_header):
        ...
        with (patch("app.middlewares.auth_middleware.UserRepository") as ur,
            ...
            ur.return_value.get_by_id = AsyncMock(return_value=user_a)
            pr.return_value.get_by_id = AsyncMock(return_value=fp)
            assert client.get(f"/api/v1/projects/{fp.id}/analysis", headers=auth_header).status_code == 404
...
class TestGetAnalysis:
    def test_404_when_no_run(self, client, user_a, sample_project, auth_header):
        with (patch("app.middlewares.auth_middleware.UserRepository") as ur,
            ...
            pr.return_value.get_by_id = AsyncMock(return_value=sample_project)
            rr.return_value.get_latest_by_type = AsyncMock(return_value=None)
            assert client.get(f"/api/v1/projects/{sample_project.id}/analysis", headers=auth_header).status_code == 404
...
class TestGeminiServiceMock:
    def test_valid_result(self, sample_project):
        r = asyncio.run(
            GeminiAgentService().analyze(sample_project, SAMPLE_REPO_ANALYSIS))
    ...
    def test_penalises_missing_tests(self, sample_project):
        ...
        r = asyncio.run(
            GeminiAgentService().analyze(sample_project, no_tests))
    ...
    def test_all_scores_in_range(self, sample_project):
        r = asyncio.run(
            GeminiAgentService().analyze(sample_project, SAMPLE_REPO_ANALYSIS))
        s = r.health_score
        for f in ("overall", "maintainability", "complexity", "test_coverage_estimate", "documentation"):
            v = getattr(s, f)
